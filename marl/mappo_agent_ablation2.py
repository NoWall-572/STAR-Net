# marl/mappo_agent_ablation2.py
# Agent for Ablation 2: ST-GCN-MAPPO
...
from .networks_ablation2 import STGCNEncoder, Actor, Critic
import config
from torch_geometric.data import Data, Batch


class MAPPOAgentAblation2:
    def __init__(self, num_agents, action_dim_uav, action_dim_ugv, device=None):
        ...
        self.encoder = STGCNEncoder(config.NODE_FEATURE_DIM).to(self.device)
    ...
    def _recompute_all_rnn_outs(self, obs_batch, hidden_batch):
        ...
        data_list = [Data(x=torch.FloatTensor(obs['node_features']), edge_index=torch.LongTensor(obs['edge_index'])) for obs in all_obs_flat]
        ...
        gcn_output = F.relu(self.encoder.gcn_conv(node_embeddings, batched_data.edge_index))
