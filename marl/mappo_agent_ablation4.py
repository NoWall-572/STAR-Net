...
# marl/mappo_agent_ablation4.py
# Agent for New Ablation 4: S-GCN-MAPPO
...
from .networks_ablation4 import SGCNEncoder, Actor, Critic
import config
from torch_geometric.data import Data, Batch


class MAPPOAgentAblation4:
    def __init__(self, num_agents, action_dim_uav, action_dim_ugv, device=None):
        ...
        self.encoder = SGCNEncoder(config.NODE_FEATURE_DIM).to(self.device)
    ...
    def update(self, buffer):
        ...
        for _ in range(config.PPO_EPOCHS):
            ...
            ratios = torch.exp(log_probs_new.mean(dim=1, keepdim=True) - action_log_probs.detach())
    ...
    def _recompute_all_encoder_outs(self, obs_batch):
        ...
        data_list = [Data(x=torch.FloatTensor(obs['node_features']),
                          edge_index=torch.LongTensor(obs['edge_index'])) for obs in all_obs_flat]
        batched_data = Batch.from_data_list(data_list).to(self.device)

        encoded_nodes = F.relu(self.encoder.node_encoder(batched_data.x))
        gcn_output = F.relu(self.encoder.gcn_conv(encoded_nodes, batched_data.edge_index))
        agent_indices = batched_data.ptr[:-1]
        agent_embeddings = gcn_output[agent_indices]
    ...
    def evaluate_actions(self, obs_batch, action_batch, avail_batch):
        ...
        gcn_output = F.relu(self.encoder.gcn_conv(encoded_nodes, batched_data.edge_index))
        agent_indices = batched_data.ptr[:-1]
        encoder_outs_flat = gcn_output[agent_indices]
