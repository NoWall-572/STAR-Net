...
def run_episode_batch_worker(agent_state_dict, num_episodes_per_worker):
    ...
    for _ in range(num_episodes_per_worker):
        ...
        for step in range(config.EPISODE_LENGTH):
            ...
            actions, action_log_probs, new_hidden_states_torch = worker_agent.choose_action(obs, hidden_states_torch,
                                                                                            avail_actions)
            ...
            env_actions = {node.id: map_action_id_to_env_action(i, actions[i], env.num_uavs) for i, node in
                           enumerate(env.nodes)}
...
def main():
    ...
    if MODEL_EPISODE_TO_LOAD > 0:
        try:
            print(f"--- Continue training the model starting from epoch {MODEL_EPISODE_TO_LOAD}.---")
            ...
            print(f"Error: The model file to be restored cannot be found. A new training session will begin.")
            MODEL_EPISODE_TO_LOAD = 0
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++

    print("--- Start Training (MAPPO) ---")
    ...
    for update_cycle in range(1, 3001):
        ...
        print(
            f"Model update completed. Duration: {update_time:.2f}s. (Critic Loss: {diagnostics['critic_loss']:.4f}, Exp_Var: {diagnostics['explained_variance']:.3f})")
        ...
        logger.log(total_episodes, total_steps,
                   {'throughput': avg_throughput, 'connectivity': avg_connectivity, 'energy_cost': avg_energy},
                   diagnostics)
        ...
        if update_cycle % SAVE_INTERVAL == 0:
            agent.save_models('./models', total_episodes)
            print(f"--- Save the model at the {total_episodes}th iteration ---")
