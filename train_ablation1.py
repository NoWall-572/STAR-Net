...
# train_ablation1.py
# Training script for Ablation 1: S-GAT-MAPPO (No Temporal Modeling) - Corrected & Robust Version
...
def run_episode_batch_worker(agent_state_dict, num_episodes_per_worker):
    try:
        env = AirGroundEnv()
        worker_agent = MAPPOAgentAblation1(
...
def main():
    print(f"--- [Ablation 1] Training will run on the device: {config.DEVICE} ---")

    NUM_WORKERS = 8
    EPISODES_PER_WORKER = 3

    env = AirGroundEnv()
    logger = Logger(log_dir='logs_ablation1')
    ...
    print("--- Begin training [Ablation 1: S-GAT-MAPPO] ---")
    ...
    for update_cycle in range(1, 3001):
        ...
        if current_batch_steps == 0:
            print("!!! Warning: All workers failed to collect any data. This update cycle is skipped. Please check worker errors.")
            continue
        ...
        if update_cycle % 10 == 0:
            agent.save_models('./models_ablation1', total_episodes)
            print(f"--- [Ablation 1] Save the model at epoch {total_episodes} ---")
