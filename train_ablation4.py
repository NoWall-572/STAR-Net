# train_ablation4.py
# Training script for New Ablation 4: S-GCN-MAPPO
...
def run_episode_batch_worker(agent_state_dict, num_episodes_per_worker):
    try:
        env = AirGroundEnv()
        worker_agent = MAPPOAgentAblation4(
...
def main():
    print(f"--- [Ablation 4] Training will run on the device: {config.DEVICE} ---")

    NUM_WORKERS = 8
    EPISODES_PER_WORKER = 3

    env = AirGroundEnv()
    logger = Logger(log_dir='logs_ablation4')
    ...
    print("--- Begin training [Ablation 4: S-GCN-MAPPO] ---")
    ...
    for update_cycle in range(1, 3001):
        ...
        if update_cycle % 10 == 0:
            agent.save_models('./models_ablation4', total_episodes)
            print(f"--- [Ablation 4] Save the model at epoch {total_episodes} ---")
