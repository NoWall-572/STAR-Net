# train_ablation5.py
# Training script for Ablation 5: No Heterogeneity Features
...
def process_obs_for_ablation5(obs_list):
    ...
    for agent_obs in obs_list:
        ...
        new_agent_obs['node_features'] = agent_obs['node_features'][:, :-2]
        processed_obs_list.append(new_agent_obs)
    return processed_obs_list


def run_episode_batch_worker(agent_state_dict, num_episodes_per_worker, ablated_feature_dim):
    try:
        env = AirGroundEnv()
        temp_config_node_feature_dim = config.NODE_FEATURE_DIM
        config.NODE_FEATURE_DIM = ablated_feature_dim

        worker_agent = MAPPOAgent(
            num_agents=env.num_agents,
            action_dim_uav=UAV_ACTION_DIM,
            action_dim_ugv=UGV_ACTION_DIM,
            device='cpu'
        )
        config.NODE_FEATURE_DIM = temp_config_node_feature_dim
        ...
        for _ in range(num_episodes_per_worker):
            obs, state = env.reset()
            processed_obs = process_obs_for_ablation5(obs)
            initial_hidden_states_torch = worker_agent.get_initial_hidden_states(processed_obs)
            ...
            for step in range(config.EPISODE_LENGTH):
                ...
                actions, action_log_probs, new_hidden_states_torch = worker_agent.choose_action(processed_obs, hidden_states_torch, avail_actions)
                ...
                local_buffer.append(
                    (processed_obs, hidden_states_np, actions, action_log_probs, info, done, state, avail_actions))

                obs = next_obs
                processed_obs = process_obs_for_ablation5(next_obs)
...
def main():
    print(f"--- [Ablation 5] Training will run on the device: {config.DEVICE} ---")

    NUM_WORKERS = 8
    EPISODES_PER_WORKER = 3

    env = AirGroundEnv()
    logger = Logger(log_dir='logs_ablation5')
    ...
    ablated_node_feature_dim = config.NODE_FEATURE_DIM - 2

    original_node_feature_dim = config.NODE_FEATURE_DIM
    config.NODE_FEATURE_DIM = ablated_node_feature_dim
    ...
    config.NODE_FEATURE_DIM = original_node_feature_dim

    total_episodes, total_steps = 0, 0

    print("--- Begin training [Ablation 5: No Heterogeneity Features] ---")
    print(f"--- Node feature dimension received by the agent: {ablated_node_feature_dim} ---")
    ...
    for update_cycle in range(1, 3001):
        ...
        with mp.Pool(processes=NUM_WORKERS) as pool:
            results = pool.starmap_async(run_episode_batch_worker, [(agent_state_dict, EPISODES_PER_WORKER, ablated_node_feature_dim) for _ in range(NUM_WORKERS)])
        ...
        for local_buffer, episode_infos in worker_outputs:
            for experience in local_buffer:
                processed_obs, hidden_np, act, log_p, info, done, state, avail = experience
                ...
                buffer.push(processed_obs, hidden_torch, act, log_p, final_reward, done, state, avail)
                current_batch_steps += 1
            all_episode_infos.extend(episode_infos)

        if current_batch_steps == 0:
            print("!!! Warning: None of the workers collected any data. This update cycle is skipped.")
            continue
        ...
        print(
            f"Data collection completed. Time taken: {collection_time:.2f}s. Total {len(all_episode_infos)} rounds collected, {current_batch_steps} steps.")
        ...
        if update_cycle % 10 == 0:
            agent.save_models('./models_ablation5', total_episodes)
            print(f"--- [Ablation 5] Save the model at epoch {total_episodes} ---")
